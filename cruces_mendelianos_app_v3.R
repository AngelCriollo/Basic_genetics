...
#               Version 3: 15 - Mayo - 2025, asistido por ChatGPT
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      textInput("n_inds", "Individuals per generation (comma-separated):", "20,20,20"),
      numericInput("n_males", "Number of male parents:", 5, min = 1),
      numericInput("n_females", "Number of female parents:", 5, min = 1),
...
server <- function(input, output) {
  output$validation <- renderText({
    ...
    n_inds <- as.numeric(strsplit(input$n_inds, ",")[[1]])
    if (length(n_inds) != input$n_gen) return("❌ Provide one size per generation.")
    if (any(is.na(n_inds)) || any(n_inds < 2)) return("❌ Invalid values in generation sizes.")
    
    if (input$n_males < 1 || input$n_females < 1) return("❌ At least one male and one female parent are required.")
  ...
  sim <- eventReactive(input$simulate, {
    ...
    males <- sample_parents(input$male_genos, input$male_props, genes, "M", input$n_males, id_counter)
    id_counter <- id_counter + input$n_males
    
    females <- sample_parents(input$female_genos, input$female_props, genes, "F", input$n_females, id_counter)
    id_counter <- id_counter + input$n_females
    ...
    for (g in 1:input$n_gen) {
      next_gen <- make_generation(prev, g, input$selfing, n_inds[g], id_counter, genes)
      all_inds <- c(all_inds, next_gen)
      prev <- next_gen
      id_counter <- id_counter + n_inds[g]
    }
